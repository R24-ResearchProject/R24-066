...
model_defects = tf.keras.models.load_model('new\model_defects2.h5')
model_seam = tf.keras.models.load_model('new\model_cloth2.h5')
...
# Function to process the camera feed and apply the models with thread pooling
def process_camera_feed(max_workers=4, frame_width=720, frame_height=800):
    cap = cv2.VideoCapture(1)  # Use 0 for the primary camera
    if not cap.isOpened():
