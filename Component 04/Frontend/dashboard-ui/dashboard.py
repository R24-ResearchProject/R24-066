...
if st.session_state['page'] == 'home':
    ...
    if worker_id_input and not worker_id_input.startswith("W_"):
        ...
        if not result.empty:
            ...
            # Streamlit button styled with the custom class
            if st.markdown('<button class="custom-button" onclick="window.location.href=\'#\'">Go to Dashboard</button>', unsafe_allow_html=True):
        ...
        with col1:
            st.markdown(
                ...
                <div class='info-card' style='border: 2px solid #333; border-radius: 10px; background-color: #2C2C2C; color: white; padding: 20px; height: 140px; position: relative;'>
                    ...
                    <div style='margin-top: 10px; font-size: 18px; font-weight: bold;'>Target: {worker_data['Production_Volume'].sum()} pcs</div>
