...
def plot_metrics(data, defect_type):
    ...
    if 'Age' in subset.columns:
        ...
        ax[0].set_title(f'Age Distribution for {defect_type}')
        ax[0].set_xlabel('Age')
    ...
    if 'Skill_Level' in subset.columns:
        ...
        ax[1].set_title(f'Skill Level Distribution for {defect_type}')
...
# Select defect type for visualization
defect_types = ['Run_Off', 'Open_Seam', 'SPI_Errors', 'High_Low']
selected_defect = st.selectbox('Select Defect Type to Display', defect_types)

# Display the plots if data is loaded and merged successfully
if not combined_data.empty:
    st.write(f"#### Defect Distribution by Age and Skill Level for {selected_defect}")
